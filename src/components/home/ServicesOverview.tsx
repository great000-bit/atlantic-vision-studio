...
import { Camera, Film, FileVideo, Mic2, Sparkles, Calendar, ArrowUpRight, LucideIcon } from "lucide-react";
...
export const ServicesOverview = () => {
  ...
  const title = getContentValue(content, 'heading', 'Complete Media Solutions');
  const subtitle = getContentValue(content, 'subheading', 'From concept to delivery, we offer end-to-end media production services.');
  const services = getContentValue(content, 'services', defaultServices) as typeof defaultServices;
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                <Link to={service.link} className="group block h-full p-8 bg-card border border-border rounded-lg hover:border-primary/50 transition-all duration-500 hover:shadow-glow">
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300"><IconComponent size={24} /></div>
